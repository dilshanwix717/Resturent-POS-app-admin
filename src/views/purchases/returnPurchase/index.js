...
const ReturnPurchase = () => {
  ...
  const cancelledGRNs = filteredGRNs.filter(grn => grn.transactionStatus === 'Cancelled');

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentGRNs = cancelledGRNs.slice(indexOfFirstItem, indexOfLastItem);
  
  const totalPages = Math.ceil(cancelledGRNs.length / itemsPerPage);
  ...
  return (
    <React.Fragment>
      <Row>
        <Card>
          <Card.Header>
            <Card.Title className="mb-3" as="h5">Returned GRN</Card.Title>
            <Row>
              <Col></Col>
              <Col>
                <Form.Control
